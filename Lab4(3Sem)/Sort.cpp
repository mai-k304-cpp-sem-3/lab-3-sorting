...
{
    transfers = 0;              // Перемещения
    comparisons = 0;            // Сравнения
    int interval = (size / 2);  // Длина промежутков между элементами

    while (interval > 0)        // Пока интервал не равен нулю (когда интервал = 0, получаем упорядоченный по возрастанию массив)
    {
        for (int i = 0; i < size - interval; i++)
        {
            int j = i;          // Запоминаем границу 
            while (j >= 0  )    // Проходим от границы до начала массива
            {
               comparisons++;
                if (arr[j] > arr[j + interval]) {   // Если         
